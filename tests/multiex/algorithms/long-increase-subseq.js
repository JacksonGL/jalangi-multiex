...
*/
  var longestSubsequence = (function () {
    ...
    function buildDag(array) {
      ...
      for (var i = 0; i < array.length; i += 1) {
        ...
        for (var j = i + 1; j < array.length; j += 1) {
          if (array[i] < array[j]) result[i][result[i].length] = j;
        }
      }
      return result;
    }
...
*/
    ...
    return function (array) {
      ...
      find.memo = [];
      for (var i = 0; i < array.length; i += 1) {
        results[results.length] = (find(dag, i));
      }
      maxPath = results[max(results, cmp)];
      results = [];
      while (maxPath) {
        results[results.length] = (array[maxPath.node]);
