...
node.prototype.merge = function (frNod, paNod, paItm) {
    ...
    this.keyval[this.keyval.length] = (del);
    for (var i=0, len=frNod.keyval.length; i<len; i++) {
        this.keyval[this.keyval.length] = (frNod.keyval[i]);
        this.nodptr[this.nodptr.length] = (frNod.nodptr[i]);
    }
    this.nodptr[this.nodptr.length] = (frNod.nodptr[frNod.nodptr.length-1]);
...
tree.prototype.pack = function () {
    ...
    while (true) {
        ...
        if (toI == 0) parNod[parNod.length] = (toN);
    ...
    for (i=1, len=parNod.length; i<len; i++) {
        parKey[parKey.length] = (parNod[i].keyval[0]);
    }
    ...
    while (parKey[0] !== null) {
        ...
        for (var i=0, len=parNod.length; i<len; i++) {
            parKey[parKey.length] = (parNod[i].keyval.pop());
        }
    }
    this.root = parNod[0];
    this.goTop();
...
tree.prototype._del = function (key) {
    ...
    if (sibR !== null && sibR.keyval.length > this.minkyl) {
        this.leaf.keyval[this.leaf.keyval.length] = (sibR.keyval.shift());
        this.leaf.recnum[this.leaf.recnum.length] = (sibR.recnum.shift());
        if (this.item == 0) this._fixNodes(stack, key, this.leaf.keyval[0]);
    ...
    while (curNod.keyval.length < this.minkyn && stack.length > 0) {
        ...
        if (sibR !== null && sibR.keyval.length > this.minkyn) {
            curNod.keyval[curNod.keyval.length] = (parNod.keyval[parItm]);
            parNod.keyval[parItm] = sibR.keyval.shift();
            curNod.nodptr[curNod.nodptr.length] = (sibR.nodptr.shift());
            break;
        }
...
var t = new tree(J$.readInput(1));
for(var i=0;i<4;i++){
    t.addKey(J$.readInput(1), J$.readInput(1));
}

for(var i=0;i<4;i++){
    t.seek(J$.readInput(1), J$.readInput(1));
}